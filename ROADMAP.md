...
- [ ] Create `src/utils/db.py` with SQLite schema: `segments` table (timestamp, camera_id, target_detected, roi_count, file_size, duration, file_path)  -  **Owner: ___________**
- [ ] Integrate database writes into the pipeline (write one row per encoded segment)  -  **Owner: ___________**
- [ ] Implement query: "return all segments from camera X where targets were detected in the last N hours"  -  **Owner: ___________**
- [ ] Write unit tests for schema creation, insertion, and query  -  **Owner: ___________**
