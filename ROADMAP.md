...
| Task | Assigned To | Due | Priority | Status | Notes |
|---|---|---|---|---|---|
| Research HLS pipeline: FFmpeg outputs .m3u8 + .ts chunks, Flask serves them | KD | 2026-04-25 | Urgent | Not Started | FFmpeg cmd: `-f hls -hls_time 2 -hls_list_size 5 -hls_flags delete_segments`. Flask serves `playlist.m3u8` + `.ts` files. |
| Integrate hls.js into index.html for browser-based HLS playback | KD | 2026-04-25 | Urgent | Not Started | Load from CDN: `https://cdn.jsdelivr.net/npm/hls.js@latest`. Use `Hls.isSupported()` to attach to `<video>`; fall back to native HLS for Safari. |
| Add RTSP URL input to GUI + validate with `ffprobe` before starting pipeline | KD | 2026-04-26 | Important | Not Started | `cv2.VideoCapture("rtsp://...")` already works in OpenCV. Hawaii traffic cams (gookami.org) are a free test target. |
| Flask HLS segment server: generate + serve playlist and .ts chunks as pipeline runs | KD | 2026-04-26 | Important | Not Started | Transcode each encoded segment to HLS chunks via subprocess FFmpeg. Serve at `/api/hls/<camera_id>/playlist.m3u8`. |
| Test HLS end-to-end: RTSP in → pipeline → HLS out → hls.js in browser | KD, RR | 2026-04-28 | Important | Not Started | A webcam streamed via VLC's RTSP server is fine for a local test. Target latency under 5 seconds. |
...
| 4.1 | HLS streaming + hls.js integration | Open 🔲 |
