...
Mobile release builds fall back to the debug key of whatever machine builds
them when `SVCS_ANDROID_KEYSTORE` is unset (see `mobile/android/app/build.gradle.kts`).
Every APK so far was built on the owner's Windows machine, so phones only
accept updates built there, and a CI- or cloud-built APK will refuse to
install as an update. Owner action: create a release keystore, back it up
somewhere safe (losing it means users must uninstall to update), and set the
four `SVCS_ANDROID_*` variables on the release machine.
...
`installer/Install-SVCS.ps1` downloads two CDnet clips from
`data/samples/cdnet_mp4/baseline/` through GitHub's LFS media URL, but no
branch has those files committed (only the README), so that step cannot
succeed. Owner action: commit the two clips with LFS, host them on a
release, or drop the option.
...
- The two blockers files (`docs/BLOCKERS.md` for current gates and
  `docs/BLOCKERS.md` for the June to September register) were merged
  into this one on 2026-09-24, so every reference points at one place.

## Register (June to September 2026)

The running log from the autonomous v2 build onward, moved here unchanged
from `docs/BLOCKERS.md`. Items marked done are kept as the record of
when each gate cleared; the open gates above are the current summary.
...
### Items logged as open in the register
...
| Checklist | Done: `docs/RELEASE-CHECKLIST.md` gained a "Mobile-only release" section so this is repeatable next time. |
...
| Item | Severity | Why deferred | Proposed approach |
|------|----------|--------------|-------------------|
| SEC-017: DNS-rebinding TOCTOU bypass of `is_safe_push_url()` (`src/utils/push_notify.py`, reused by `src/utils/event_webhook.py`) | Medium (fixed 2026-09-26) | The guard validated one DNS resolution of the target hostname; the actual outbound request re-resolved the hostname independently, so an attacker-controlled DNS answer could differ between the two lookups and bypass the SSRF block. Proven locally with a mocked resolver, not a live network attack. | Fixed: `validate_push_url()` now returns the addresses it approved and `pin_resolution()` pins the actual connection to them for the request's lifetime, so only one resolution is ever in effect. See `tests/security/test_dns_rebinding.py` and `docs/SECURITY.md` SEC-017. |
...
Authors: Bloodawn (KheivenD), 2026-06-02 (register), 2026-09-21 (current
gates), 2026-09-24 (merged).
