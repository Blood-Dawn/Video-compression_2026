...
- [x] **What:** Make a fresh install genuinely start blank, and give a way to reset on a machine that already has state. *(DONE: CPU averages loaded from a prior install are flagged historical and the Metrics panel labels them "(previous runs)" so a fresh install shows nothing live; cpu_sampler gained reset_mode_avgs() and session-key tracking. paths.reset_state() deletes the per-install state files (flask_secret, mode_cpu_avgs.json, gui_state.json, usage_consent.json, usage_events.jsonl). New POST /api/setup/reset clears those files, the in-memory CPU panel, and the destination choice, returning the app to first-run (route guards 58/15). A "Reset app data (factory reset)" button plus an "Open Setup" button were added to the Help overlay; setup.js resetAppData() confirms then reloads to re-trigger Setup. installer/svcs.iss uninstaller now OFFERS (opt-in MsgBox) to remove %LOCALAPPDATA%/SVCS. tests/test_reset.py covers reset_state, reset_mode_avgs, and the reset route. Browser-verified: Help shows the Reset button and the route flips setup_complete back to false and removes the state files. Suite green: 880 passed.)*
...
- [x] **What:** The "Local RTSP Server" and "Live Stream (HLS)" blocks currently sit in the left sidebar. Move them into a new topbar tab next to Metrics/Search. Call it **TOOLS** (or "ADVANCED"). *(DONE: new TOOLS nav button placed between SEARCH and ENCRYPT (blue active accent) and a #tab-tools page. Rather than risky cut-paste of the glyph-heavy markup, new tools.js relocates the existing DOM nodes (the HLS-controls config-section, the #rtsp-server-details config-section, and the #hls-section player) into #tab-tools-body at load time, so every id and onclick handler is preserved and switchTab stays generic. The sidebar no longer shows them; no API/route change (guards stay at 58). tests/test_tools_tab.py + updated test_upload_tab nav order. Browser-verified via the Preview MCP: nav order home/upload/metrics/search/tools/encrypt, clicking TOOLS shows the relocated HLS + RTSP controls and player (confirmed they are inside #tab-tools-body and no longer in .sidebar), hlsStart/rtspStart still defined, no console errors. Suite green: 888 passed.)*
