...
- [x] **Owner doc.** A short note in the test's module docstring (and a line in `docs/FEATURE-AUDIT.md`): "Runs the real pipeline on the CDnet clips in `data/samples/cdnet_mp4` (or set `SVCS_TEST_VIDEO_DIR` to point at any folder of clips). Run `uv run --no-sync pytest tests/test_real_videos.py -s` to see real compression ratios per clip and mode, and confirm each mode + codec works on real footage." No new media committed; it reuses the clips already in the repo.
