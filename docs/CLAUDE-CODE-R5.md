...
- [x] **What:** the SQLite metadata DB already records object class, color, scene, camera, and time per event. Add a light query layer that turns a phrase like "red car after 9pm on cam2" into structured filters (class=car, color=red, hour>=21, camera=cam2) and returns matching events, plus a SEARCH-tab UI to type the phrase and see results with thumbnails. This is the self-hosted, no-cloud answer to the "natural-language search" that current NVR products advertise, and it uses data you already have. *(DONE 2026-08-16: gui/services/nl_query.py deterministic parser (classes, colors, scenes, time-of-day, dayparts, after/before/between hours, ISO + today/yesterday dates, cameras, count comparisons) + GET /api/nl_search on the queries blueprint returning results plus an "explain" echo. Unknown words degrade to a LIKE match; every value is SQL-bound and a hostile metacharacter phrase is covered by test. Route guard 78 -> 79. tests/test_nl_query.py: 19 tests (eleven-phrase acceptance table, DB-level correctness, SQL-injection safety, hidden-row exclusion, old-schema migration). UI DONE 2026-08-16: SEARCH tab gained a Smart search bar + explain echo + results table (index.html + encryption.js runNlSearch), browser-verified end to end: "red car on the highway at night" parsed to class/color/scene/time filters and rendered the matching row. The route also migrates an old-schema archive DB idempotently before querying, which a real deployment DB needed.)*
...
- [x] **What:** on finalize, write a SHA-256 of each compressed output plus a signed-style manifest (a hash chain linking outputs in order), and add a `verify` action that re-checks a folder against its manifest. Optionally burn a timestamp into a corner of the output. This makes a clip defensible as evidence, which is unusual for a free tool and fits SVCS's origin. *(DONE 2026-08-16: src/utils/integrity.py, MANIFEST.sha256.jsonl hash chain appended after every ffprobe-verified auto-compress output; verify_manifest() + CLI "python -m utils.integrity verify <folder>". A flipped byte names the file; an edited manifest line breaks the chain at that index. Best-effort by design, a manifest failure never fails a successful compress. Integrity-not-authenticity limitation stated in the docstring. tests/test_integrity.py: 8 tests. NOT DONE: the optional timestamp burn-in and a UI verify button.)*
