...
**Docker note (planner TASK 3.16, fixed 2026-09-22):** the server-scenario
image (`Dockerfile`) no longer needs this run manually. A throwaway builder
stage runs this exact export automatically during `docker compose up --build`
(CPU-only torch, discarded after the copy), so a genuinely clean clone builds
successfully with no prerequisite step. This section's manual command is still
how a local dev/source checkout (`uv run python run_gui.py`) gets its
`yolov8n.onnx`.
  ```
