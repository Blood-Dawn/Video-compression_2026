...
Fixes applied by KD (Apr 13 2026):
  - save_csv: collect all unique fieldnames across frame and ROI result dicts
    to prevent ValueError when frame results lack the 'roi_size' key
  - save_csv / notebook: mkdir(parents=True, exist_ok=True) ensures outputs/
    directory is created on fresh clones
...
def benchmark_upscale_roi(enhancer: Enhancer, n_frames: int) -> list:
    """
    Time upscale_roi at each (resolution x ROI size) combination.
...
def save_csv(results: list, path: str):
    ...
    Collects all unique fieldnames across every result dict so that mixed
    upscale_frame (no roi_size) and upscale_roi (has roi_size) rows can be
    written to a single file without a ValueError.
    """
    if not results:
        return
    Path(path).parent.mkdir(parents=True, exist_ok=True)

    # Preserve insertion order; collect keys from every row
    seen: dict = {}
    for r in results:
        for k in r.keys():
            seen[k] = None
